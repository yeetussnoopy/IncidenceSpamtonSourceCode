local mariActor, super = Class(ActorSprite)

function mariActor:init(actor)
    super:init(self, actor)

    self.body = SpamtonPart("npcs/mari/body", 14, 25, 0, 0.33)
    self.body.id = "body"
    self.body.layer = -5
    self.body.swing_range = {math.rad(-7.5),math.rad(7.5)}
    self:addChild(self.body)

    self.leg_l = SpamtonPart("npcs/mari/leg_l", 16, 49, 0.5, 0)
    self.leg_l.id = "leg_l"
    self.leg_l.layer = -10
    self:addChild(self.leg_l)

    self.leg_r = SpamtonPart("npcs/mari/leg_r", 25, 48, 0.5, 0)
    self.leg_r.id = "leg_r"
    self.leg_r.layer = -10
    self:addChild(self.leg_r)

    self.arm_l = SpamtonPart("npcs/mari/arm_l", 16, 27, 0, 0.5, math.pi/2)
    self.arm_l.id = "arm_l"
    self.arm_l.layer = -12
    self:addChild(self.arm_l)

    self.arm_r = SpamtonPart("npcs/mari/arm_r", 33, 29, 0, 1, math.pi/2)
    self.arm_r.id = "arm_r"
    self.arm_r.layer = -4
    self:addChild(self.arm_r)

    self.wing_l = SpamtonPart("npcs/mari/wing_l", -3, 4, 1, 0.5)
    self.wing_l.id = "wing_l"
    self.wing_l.layer = -15
    self:addChild(self.wing_l)

    self.wing_r = SpamtonPart("npcs/mari/wing_r", 30, 8, 0, 0.5)
    self.wing_r.id = "wing_r"
    self.wing_r.layer = 5
    self:addChild(self.wing_r)

    self.head = mariHead(15, 6)
    self.head.id = "head"
    self.head.layer = -15
    ...
    for i=1, 1 do
        local str = SpamtonString(bg_sx * (1.5 + (i+4)/15) - ox/2 - 5, bg_sy, true)
        self:addChild(str)
    ...
    for i=1,1 do
        ...
        local str = SpamtonString((fg_sx / 1.2) + ((i-1)*2.5), fg_sy, false, (fg_sx / 1.5) + ((i-1)*4))
        self:addChild(str)
...
function mariActor:update()
    super:update(self)

self.rtimer = self.rtimer + (1 * DTMULT)

    self.rotation=math.rad(10*math.cos(self.rtimer * 0.05) * 0.7)
    end

-- heck it, custom version
function mariActor:setAnimation(anim, ...)
    if not ignore_actor then
        self.actor:onSetAnimation(self, anim, ...)
    end
end

function mariActor:getPart(name)
...
function mariActor:getPartIndex(part)
...
function mariActor:setPartSprite(name, path)
    local part = self:getPart(name)
    part:setSprite(path)
end

function mariActor:setPartRotation(name, rotation)
    local part = self:getPart(name)
    part.swing_rotation = rotation
end

function mariActor:tweenPartRotation(name, rotation, time, ease)
    local part = self:getPart(name)
    self.timer:tween(time or 0.5, part, {swing_rotation = rotation}, ease or "linear")
end

function mariActor:setPartScale(name, scale_x, scale_y)
    local part = self:getPart(name)
    part:setScale(scale_x, scale_y or scale_x)
end

function mariActor:tweenPartScale(name, scale_x, scale_y, time, ease)
    local part = self:getPart(name)
    self.timer:tween(time or 0.5, part, {scale_x = scale_x, scale_y = scale_y or scale_x}, ease or "linear")
end

function mariActor:setPartShaking(name, shake)
    local part = self:getPart(name)
...
function mariActor:setPartSwingShaking(name, shake)
    local part = self:getPart(name)
    if type(shake) == "number" then
...
function mariActor:setPartSwingSpeed(name, speed, keep_spin)
    local part = self:getPart(name)
...
function mariActor:setPartSwingRange(name, min, max)
    local part = self:getPart(name)
    if not max then
...
function mariActor:tweenPartSwingRange(name, min, max, time, ease)
    local part = self:getPart(name)
    if not max then
        min, max = -min, min
    end
...
function mariActor:setPartSine(name, value)
    local part = self:getPart(name)
    part.siner = value or 0
end

function mariActor:setHeadFrame(num)
    self.head.animating = false
    self.head:setFrame(num)
end

function mariActor:setHeadAnimating(state)
    self.head.animating = state ~= false
end

function mariActor:setAllPartsShaking(shake)
...
function mariActor:setSwingSpeed(speed)
    self.swing_speed = speed
end

function mariActor:setSwingRange(min, max)
    if not max then
...
function mariActor:setStringCount(num)
    if num < 4 then
...
function mariActor:snapString(index, remove)
    local str
    if index then
...
function mariActor:snapStrings(count, remove)
    for i=1,count do
        self:snapString(nil, remove)
    end
end

function mariActor:resetPart(name, full)
    local part = self:getPart(name)
    part:reset(full, self:getPartIndex(part))
end

function mariActor:resetParts(full)
    self.swing_speed = 1
    for i,part in ipairs(self.parts) do
        part:reset(full, i)
    end
